...
To test the expandToInclude method, 3 tests were created. The first test is to check if the method finds includes the input value in the mentioned range, the test failed. The second test is to check if the method expands the upper bound of the range to include the input value, however, this test also failed. Finally the third test is to check if the mothod expands the lower bound of the range to include the input value, this test also failed.
